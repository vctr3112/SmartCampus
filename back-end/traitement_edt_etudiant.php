...
if ($etudiant = mysqli_fetch_assoc($result_user)) {
    ...
    $donnees_edt["numero_etudiant"] = $etudiant["numero_etudiant"];
}


$sql_seances = "SELECT c.nom_cours, 
                       DATE_FORMAT(s.date, '%d/%m/%Y') as date_fr,
                       DATE_FORMAT(s.heure_debut, '%H:%i') as heure_debut,
                       DATE_FORMAT(s.heure_fin, '%H:%i') as heure_fin,
                       s.salle 
                FROM Seance s 
